...
export const stepSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("fill"), selector: z.string(), value: z.string().describe("Sets an input/textarea's value. Framework-aware: fires the React-safe native setter + input/change/blur, so controlled forms (React, react-hook-form, Formik, Vue) register the value AND mark the field touched. This is the fix for 'I filled the field but submit says it's still empty' — always use fill for form fields, not evaluate."), force: z.boolean().optional().describe("Bypass the honeypot guard. fill refuses fields no human can reach (offscreen/clipped/transparent) — only set this when you are certain the field is real.") }),
